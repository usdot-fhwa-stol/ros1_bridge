...
def generate_cpp(output_path, template_dir):
    ...
    data.update(generate_services(rospack, message_string_pairs=message_string_pairs))
    data.update(generate_actions(rospack, message_string_pairs=message_string_pairs))
...
def determine_common_actions(
    ros1_actions: list[Message],
    ros2_actions: list[Message],
    mapping_rules,
    ...
    for pair in pairs:
        ...
        if force:
            ...
            ros2_fields["goal"] = [
                a
                for a in ros2_spec.goal.fields
                for b in ros1_spec.goal.fields()
                if a.name == b[1]
            ]
            ros2_fields["result"] = [
                a
                for a in ros2_spec.result.fields
                for b in ros1_spec.result.fields()
                if a.name == b[1]
            ]
            ros2_fields["feedback"] = [
                a
                for a in ros2_spec.feedback.fields
                for b in ros1_spec.feedback.fields()
                if a.name == b[1]
            ]
...
def determine_field_mapping(
    ros1_msg, ros2_msg, mapping_rules, msg_idx, allow_missing=False
):
    """
